...
def clean_text_en(text):
    ...
    text = re.sub(r"[;\"'()\[\]{}<>\@$]", "", text)  # Remove punctuation
    text = text.replace('\u200d', '').replace('\u200c', '').replace('\u200b', '')
    # remove non hindi, non numeric characters
    text = re.sub(r'[^a-zA-Z0-9 ,\.।॥%:;?!\'"-]', '', text)
    # multiple space to be replaced by one
    text = re.sub(r'\s+', ' ', text)
    return text

def transliterate_all(text, lang = 'Devanagari'):
    return transliterate.process(lang, 'ITRANS', text)
...
class BPETokenizer(PreTrainedTokenizer):
    def __init__(
        ...
        **kwargs
        ...
        self.name_or_path = "itrans-tokenizer"
    ...
    def train_tokenizer(cls, 
        ...
        # Define files iterator to process all text files in the provided folders
        def files_iterator():
            for folder in corpus_folders:
                for filename in os.listdir(folder):
                    if filename.endswith('.txt'):
                        ...
                        clean_lang = 'hi' if 'hindi-wiki' in folder else 'mr' if 'marathi-wiki' in folder else 'en'
                        is_ipa = 'ipa' in folder.lower()  
                        
                        try:
                            with open(file_path, 'r', encoding='utf-8') as f:
                                for line in f:
                                    if line.strip():
                                        if clean_lang in ['hi', 'mr'] and not is_ipa:
                                            yield transliterate_all(clean_text(line.strip(), clean_lang), 'Devanagari')
                                        elif is_ipa:
                                            yield clean_text(line.strip(), clean_lang)
                                        else:
                                            yield clean_text_en(line.strip())
            ...
            for text in ds['train']['context']:
                if text.strip():
                    yield clean_text_en(text.strip())
...
def main():
    """Train a ITRANS tokenizer and save it."""
    parser = argparse.ArgumentParser(description="Train a ITRANS tokenizer")
    ...
    parser.add_argument("--output_dir", type=str, default = "./all_tokenizer_files/itrans_tokenizer_files_ehm_ipa", help="Directory to save the tokenizer")
    parser.add_argument("--vocab_size", type=int, default=17411, help="Size of the vocabulary")
    parser.add_argument("--min_frequency", type=int, default=2, help="Minimum frequency for a token")
    ...
    print(f"Training ITRANS tokenizer on folders: {', '.join(args.corpus_folders)} with vocab size {args.vocab_size}...")
    ...
    # Test the tokenizer if a test text is provided
    if args.test_text:
        print("\nTesting tokenizer:")
        ...
        # Verify spacing in decoded text
        print("\nVerifying spacing in decoded text:")
        for i, (original_word, decoded_word) in enumerate(zip(args.test_text.split(), decoded.split())):
            print(f"Word {i+1}: Original='{original_word}', Decoded='{decoded_word}'")
